...
pprob <- cbind(pspnewdata, mu=ypred$fit, se=ypred$se.fit) %>% 
    left_join(read.csv("../Data/spnames.csv")) %>% 
  ggplot(aes(fct_reorder(plotname, mu), mu))+
  ...
  scale_color_manual(values = pcols, labels = c("Rainy","Dry"))+
  scale_shape_discrete(labels = c("Rainy","Dry"))+
  theme_classic(base_size = 14)+
  theme(axis.text.x = element_text(angle = -45, hjust=0), legend.position = 'top')+
  labs(x = "Species", y="Crossing probability", shape="Season",color="Season")
#png("figures/fig3_prob.png", width = 6, height = 5, units = 'in', res = 300)
#pprob
#dev.off()
```
