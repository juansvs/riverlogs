...
pfreqs2 <- cbind(basefreqnewdata, mu=basefreqpred$fit, se=basefreqpred$se.fit) %>% 
  ...
  geom_pointrange(aes(ymin = mu-2*se, ymax = mu+2*se, color=fct_relevel(season,c("dry","rainy")),shape=fct_relevel(season,c("dry","rainy"))), position = position_dodge(0.3), show.legend = c(shape=T,color=F))+
  scale_color_manual(values = pcols, labels = c("Dry","Rainy"))+
  theme_classic(base_size = 12)+
  theme(axis.text.x = element_text(angle = -45, hjust=0), legend.position = 'top')+
  labs(x = "Species", y="Crossing frequency (/week)", shape="Season")
```
