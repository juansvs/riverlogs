...
The model seems to be zero-inflated. It is recommended to use a quasi-Poisson error correction or negative binomial distribution instead.

#### Quasi-Poisson model

```{r}
freqmodels_nb <- list()
freqmodels_nb$full_RndIntSsn <- glmer.nb(n ~ Log_lenght+Diameter+Log_to_water+River_depth+(1+season|species), data = dat_glmm_week_freq)
summary(freqmodels_nb$full_RndIntSsn)
```

```{r}
check_model(freqmodels_nb$full_RndIntSsn)
```
#### Zero-inflated model

```{r}
library(glmmTMB)
zifreqmodel <- glmmTMB(formula(freqmodels$full_RndIntSsn), data = dat_glmm_week_freq, family = poisson, ziformula = ~.)
zifreqmodel2 <- update(zifreqmodel, ~ Log_lenght+Diameter+Log_to_water+River_depth+season+(1|species))
anova(zifreqmodel, zifreqmodel2)
```
The model with only random intercept provides lower AIC than the one with random intercept and slope for season. 
```{r}
summary(zifreqmodel2)
```
