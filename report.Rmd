...
```{r}
filter(dat_raw, !DeleteFlag) %>% 
  ggplot(aes(DateTime))+geom_density()
```
...
There seem to be some important differences in the probability of crossing across species, some cross nearly every occasion while others rarely do. 
...
There were 1096 independent events, so 1096/4885 trap days = 0.224 /day, but this varied across stations, between 0.02 and 1.4 events/day.

```{r}
dat %>% group_by(station) %>% mutate(stn_effort = as.numeric(max(DateTime)-min(DateTime))) %>% summarize(n = n(), freq = n()/mean(stn_effort),prop = sum(eventcross)/n())
```
...
dat %>% filter(eventcross) %>% mutate(weekid = ceiling(difftime(DateTime,min(DateTime), units = "week"))) %>% 
  mutate(week_yr = week(DateTime)) %>% count(station,species,weekid) %>% # group by week of the year
  ggplot(aes(weekid,n))+geom_point(aes(color = species))
```

Below I create a database with the number of weekly detection for every station and species

```{r}
temp <- dat %>% mutate(yr = year(DateTime), wk = week(DateTime)) %>% distinct(station,yr,wk) %>% 
  group_split(station) %>% sapply(\(x) data.frame(station = unique(x$station), ))
mutate(week = min(yr+wk):max(yr+wk))
  (station) %>% summarise(minw = min(wk+yr),maxw=max(wk+yr))
