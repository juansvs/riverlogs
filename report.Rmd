...
dat_raw <- read_csv("timelapse_database.csv")
...
log_data <- readxl::read_excel("covariates_data_base.xlsx")
...
dat_glmm_week_freq <- dat %>%  
  filter(n()>10, eventcross, .by = species) %>% 
...
Conclusion. I fit a negative binomial mixed effects model with the number of detections as a response variable. I included a random intercept species. I included the log length, log width, height above water, stream width, stream depth, and season as fixed effects.
...
Plot of crossing frequency (per week), for each species (x) in the rainy and dry seasons (color, shape).

```{r}
# pcols <- c("gold2","dodgerblue")
pcols <- c("orange","darkgreen")
left_join(dat_glmm_week_freq, read.csv("../Data/spnames.csv")) %>% 
  ggplot(aes(fct_reorder(plotname, n, max), n, color = season, shape = season))+
  # geom_boxplot(outlier.shape = NULL)+
  # geom_point(position = position_jitterdodge())+
  geom_count(position = position_dodge(0.7))+
  labs(x = "Species", y = "Crossing frequency (/week)", color = "Season", shape = "Season")+
  scale_color_discrete(type = pcols, labels = c("Dry","Rainy"))+
  scale_shape_discrete(labels = c("Dry", "Rainy"))+
  theme_classic()+
  theme(axis.text.x = element_text(angle = -45, hjust = 0),
        plot.margin = unit(c(1,5,1,1), "mm"), 
        panel.border = element_rect(fill=NA), 
        legend.position = 'top') -> pfreqs_data
```

Plots of crossing frequency wrt to log covariates.

```{r}
dat_freq_ogscale <- mutate(dat_glmm_week_freq, 
       Log_length = unstandardise(Log_lenght),
       Log_width = unstandardise(Diameter),
       Height = unstandardise(Log_to_water), 
       Depth = unstandardise(River_depth),
       River_width = unstandardise(Stream_width))
plen_data <- ggplot(dat_freq_ogscale, aes(Log_length, n, shape=season,color=season))+
  geom_point(position = position_jitter(), shape = 1)+
  labs(x = "Log length (m)", y = "Frequency (/week)")
pwid_data <- ggplot(dat_freq_ogscale, aes(Log_width, n,shape=season,color=season))+
  geom_point(position = position_jitter(), shape = 1)+
  labs(x = "Log width (cm)", y = "Frequency (/week)")
pht_data <- ggplot(dat_freq_ogscale, aes(Height, n,shape=season,color=season))+
  geom_point(position = position_jitter(), shape = 1)+
  labs(x = "Height above water (m)", y = "Frequency (/week)")
pdep_data <- ggplot(dat_freq_ogscale, aes(Depth, n,shape=season,color=season))+
  geom_point(position = position_jitter(), shape = 1)+
  labs(x = "Stream depth (cm)", y = "Frequency (/week)")
prwid_data <- ggplot(dat_freq_ogscale, aes(River_width, n, shape=season,color=season))+
  geom_point(position = position_jitter(), shape = 1)+
  labs(x = "Stream width (m)", y = "Frequency (/week)")
cowplot::plot_grid(plen_data, pwid_data, pht_data, pdep_data, labels = c('b','c','d','e'))
```

```{r}
png("../figures/fig2_freq_data.png", width = 210, height = 148, units = "mm", res = 300)
cowplot::plot_grid(pfreqs_data+theme(aspect.ratio = 1, plot.margin = unit(c(0,15,0,2), units = 'mm')),  labels = 'a', cowplot::plot_grid(plen_data, pwid_data, pht_data, pdep_data, labels = c('b','c','d','e')), rel_widths = c(4,5))
dev.off()
```

##### Model predictions

Model predicted (mean +- se) crossing frequency for different species in dry and rainy season.
...
pfreqs2 <- cbind(basefreqnewdata, mu=basefreqpred$fit, se=basefreqpred$se.fit) %>% 
  ...
  geom_pointrange(aes(ymin = mu-2*se, ymax = mu+2*se, color=fct_relevel(season,c("dry","rainy")),shape=fct_relevel(season,c("dry","rainy"))), position = position_dodge(0.3))+
  scale_color_manual(values = pcols, labels = c("Dry","Rainy"))+
  scale_shape_discrete(labels = c("Dry","Rainy"))+
  theme_classic(base_size = 12)+
  theme(axis.text.x = element_text(angle = -45, hjust=0), legend.position = 'top')+
  labs(x = "Species", y="Crossing frequency (/week)", shape="Season", color = "Season")
```

Model predictions, changing one covariate while keeping the others at their mean.
...
# Log length
pspnewdata <- data.frame(Log_lenght = seq(min(bestfreqmod$frame$Log_lenght),    max(bestfreqmod$frame$Log_lenght),
                                            length.out = 100), 
           Diameter = 0, 
           Log_to_water = 0, 
           Stream_width = 0, 
...
#Diameter
pspnewdata <- data.frame(Diameter = seq(min(bestfreqmod$frame$Diameter),
                                            max(bestfreqmod$frame$Diameter),
                                            length.out = 100), 
           Log_to_water=0,
           Log_lenght = 0, 
           Stream_width = 0, 
...
# River depth
pspnewdata <- data.frame(River_depth = seq(min(bestfreqmod$frame$River_depth),
                                            max(bestfreqmod$frame$River_depth),
                                            length.out = 100), 
           Log_to_water = 0,
           Diameter = 0, 
           Log_lenght = 0, 
...
# River width
pspnewdata <- data.frame(Stream_width = seq(min(bestfreqmod$frame$Stream_width),
                                            max(bestfreqmod$frame$Stream_width),
                                            length.out = 100), 
           Log_to_water = 0,
           Diameter = 0, 
           Log_lenght = 0, 
...
# new limits so that graphs are comparable
newylims = scale_y_continuous(limits = c(0.16,0.72))
```

Combined plots with data points and predicted lines.

```{r}
# Join the plots with the predictions for the rainy season (ie the higher prediction) with the actual data.
p2b <- plen+geom_point(data=dat_freq_ogscale, aes(Log_length, n, shape=season,color=season),position = position_jitter(), show.legend = F)+scale_color_discrete(type = pcols)#+scale_shape_discrete(solid = F)
p2c <- pwid+geom_point(data=dat_freq_ogscale, aes(Log_width, n, shape=season,color=season),position = position_jitter(), show.legend = F)+scale_color_discrete(type = pcols)#+scale_shape_discrete(solid = F)
p2d <- pht+geom_point(data=dat_freq_ogscale, aes(Height, n, shape=season,color=season),position = position_jitter(), show.legend = F)+scale_color_discrete(type = pcols)#+scale_shape_discrete(solid = F)
p2e <- pdep+geom_point(data=dat_freq_ogscale, aes(Depth, n, shape=season,color=season),position = position_jitter(), show.legend = F)+scale_color_discrete(type = pcols)#+scale_shape_discrete(solid = F)

```

Export model predictions to figure.

```{r}
png("figures/fig2_modpreds.png", width = 10.5, height = 6, units = "in", res = 300)
ggarrange(pfreqs2+theme(plot.margin = unit(c(5,5,5,5),"mm")),
          ggarrange(plen+newylims,pwid+newylims, pht+newylims, pdep+newylims, 
                    align = 'hv', labels = letters[2:5])+theme(plot.margin = unit(c(3,4,3,3),"mm")),
          labels='a',
          widths = c(0.75,1), heights = c(0.75,1))
dev.off()
```

Export combined figures (data points and model predicted lines)

```{r}
png("figures/fig2_freqmodpreds.png", width = 10.5, height = 6, units = 'in',res = 300)
ggarrange(pfreqs_data+theme(plot.margin = unit(c(0,1,0,0),"cm")),
          # plot_grid(plen+newylims,pwid+newylims,pht+newylims,pdep+newylims,prwid+newylims,
          ggarrange(p2b,p2c,p2d,p2e,
                    align = 'hv', labels = letters[2:6]),
          labels='a',
          rel_widths = c(0.75,1), scale = c(0.9,1))
dev.off()
```
...
dat_glmm <- dat_wcovs_scaled %>% 
  filter(station %in% c("CLOG03", "CLOG07", "CLOG08", "CLOG09", "PLOG02", "PLOG04")) %>% # only keep stations overlooking the near bank
  mutate(month = month(DateTime), season = if_else(lubridate::month(DateTime) %in% c(1:4,12),"dry","rainy")) %>% left_join(rainfall) %>% 
  filter(n()>10, .by=species) 
...
The model performs quite poorly, there is significant VIF, probably due to the low number of logs. Let's reduce the number of variables. I will do this stepwise, removing first the covariates with the highest VIF.

```{r}
check_collinearity(mm1)
```

Log diameter and length have very high VIF, we remove diameter first.

```{r}
check_collinearity(update(mm1, .~.-Diameter))
```

Following the same process, we eliminate also log length and river depth.

```{r}
check_collinearity(update(mm1, .~.-Diameter-Log_lenght-River_depth))
```

```{r}
summary(update(mm1, .~.-Diameter-Log_lenght-River_depth))
```

This model does not have strong collinearities, but we can see if there is a more parsimonious model, we remove the covariate with the lowest z value.

```{r}
AIC(mm1,
    update(mm1, .~.-Diameter-Log_lenght-River_depth),
    update(mm1, .~.-Diameter-Log_lenght-River_depth-Stream_width))
```

```{r}
summary(update(mm1,.~.-Diameter-Log_lenght-River_depth-Stream_width))
```

This model has a lower AIC (\delta AIC = 3.7) than the saturated model, and than the one with low collinearity (\delta AIC = 2.0). Removing one additional non-significant variable does not improve the fit. The most parsimonious model is therefore the one with fixed effects of season and height above water.

```{r}
summary(update(mm1,.~.-Diameter-Log_lenght-River_depth-Stream_width-Log_to_water))
```

```{r}
compare_performance(mm1,                     update(mm1,.~.-Diameter-Log_lenght-River_depth-Stream_width),                    update(mm1,.~.-Diameter-Log_lenght-River_depth),                  update(mm1,.~.-Diameter-Log_lenght),
                    update(mm1, .~.-Diameter),                    update(mm1,.~.-Diameter-Log_lenght-River_depth-Stream_width-Log_to_water),                     update(mm1,.~.-Diameter-Log_lenght-River_depth-Stream_width-Log_to_water-season), metrics = 'common')
```

```{r}
mm3 <- glmmTMB(eventcross~Log_to_water+season+(1|species), data=dat_glmm, family = "binomial")
summary(mm3)
```

The best performing model is the one with only height above water and season, and only season has a significant effect. The likelihood of crossing is higher during the rainy season.

#### Baseline crossing probability

```{r}
(basecrossprob <- predict(mm3, newdata=data.frame(season=c('rainy',"dry"), Log_to_water=0), se.fit=T,type='response', re.form = NA))
```

The baseline probability for all species is high, around 0.76, but it also depends on the species.
...
pprobdat <- cbind(pspnewdata, mu=ypred$fit, se=ypred$se.fit) %>% 
    left_join(read.csv("../Data/spnames.csv")) 
pprob <- ggplot(pprobdat,aes(fct_reorder(plotname, mu), mu))+
  geom_hline(yintercept = basecrossprob$fit, linetype=2, color = rev(pcols))+
  geom_pointrange(aes(ymin = mu-se, ymax = mu+se, color=fct_relevel(season,c("dry","rainy")),shape=fct_relevel(season,c("dry","rainy"))), position = position_dodge(0.3))+
  scale_color_manual(values = pcols, labels = c("Dry","Rainy"))+
  scale_shape_discrete(labels = c("Dry","Rainy"))+
  # theme_classic(base_size = 14)+
  theme(axis.text.x = element_text(angle = -45, hjust=0), legend.position = 'top')+
  labs(x = "Species", y="Crossing probability", shape="Season",color="Season")
pprob
```

```{r}
png("figures/fig3_prob.png", width = 8, height = 10, units = 'cm', res=300)
pprob+theme_classic()+theme(legend.position = 'top', axis.text.x = element_text(angle = -45, hjust = 0))
dev.off()
```
...
```{r}
summarise(dat_glmm, prop = sum(eventcross)/length(eventcross), .by = c(species, season))
ggplot(dat_glmm)+geom_count(aes(species,eventcross, color=season), position = position_dodge(1))
```
