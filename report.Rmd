...
log_data_scaled <- mutate(log_data, across(Log_lenght:Vel_conver,scale))
...
We analyze the number of animals seen crossing. For this, we divide the detection into weekly detection frequencies.
...
```{r}
temp <- dat %>% mutate(week = week(DateTime)) %>% 
  group_by(station) %>% summarise(minw = min(week),maxw=max(week))
stn_sp_weeks <- lapply(1:nrow(temp),\(x) data.frame(station = temp$station[x], week = temp$minw[x]:temp$maxw[x])) %>% 
  do.call(rbind,.) %>% 
  full_join(distinct(select(dat, station, species)))
rm(temp)
```
...
dat_glmm_week_freq <- dat %>% group_by(species) %>% filter(n()>10)%>% filter(eventcross) %>% ungroup() %>% 
  ...
  count(station,species,week) %>% 
  right_join(stn_sp_weeks) %>% 
  mutate(n = replace_na(n,0)) %>% 
  mutate(month = week%/%4.3, season = if_else(week>16&week<48, "rainy","dry")) %>% 
...
freqmm <- glmer(n ~ Canopy_per+Log_lenght+Diameter+season+Log_to_water+River_depth+(1|species), data = dat_glmm_week_freq, family = poisson())
summary(freqmm)
```

The model seems to be zero-inflated. It is recommended to use a negative binomial distribution instead.

```{r}
performance::check_zeroinflation(freqmm)
```

```{r}
freqmm_negbin <- glmer(n ~ Canopy_per+Log_lenght+Diameter+season+Log_to_water+River_depth+(1|species), data = dat_glmm_week_freq, family = negative.binomial(1))
performance::check_zeroinflation(freqmm_negbin)
```

```{r}
summary(freqmm_negbin)
```

```{r}
margins::margins_summary(freqmm_negbin)
```
...
```{r}
plot(margins::margins(freqmm_negbin))
```
...
```{r}
freqmod_nornd <- glm(n ~ Canopy_per+Log_lenght+Diameter+season+Log_to_water+River_depth, data = dat_glmm_week_freq, family = poisson())
...
```{r}
freqmm_rain <- glmer(n ~ Canopy_per+Log_lenght+Diameter+rain_sc+Log_to_water+River_depth+(1|species), data = dat_glmm_week_freq, family = poisson())
summary(freqmm_rain)
```
...
In this model the contribution of the random effect is small, around 3%.

```{r}
performance::r2(freqmm_rain)
```

This model is also not explaining the variation well, the marginal R\^2, corresponding to the fixed effects, is 0.096.
...
dat_wcovs_scaled <- dat_wcovs %>% mutate(month = month(DateTime), season = if_else(lubridate::month(DateTime) %in% c(1:4,12),"dry","rainy")) %>% left_join(rainfall)
# filter to keep only species with more than 10 records
dat_glmm <- dat_wcovs_scaled %>% group_by(species) %>% filter(n()>10) %>% ungroup() %>% mutate(month = month(DateTime)) %>% left_join(rainfall) %>% mutate(season = if_else(month>4 & month<12, "rainy","dry"), rain = scale(rain))
# Build model with season as a covariate, a random effect of species, and multiple covariates
mm1 <- glmer(eventcross~Log_to_water+Diameter+Log_lenght+Canopy_per+River_depth+rain+(1|species), data=dat_glmm, family = "binomial")
...
The second model, without season, performs better than the one with. The difference in AIC is less than 2, so the mothers are actually comparable.
...
```{r}
exp(fixef(mm1)[1])/(1+exp(fixef(mm1)[1]))
```
