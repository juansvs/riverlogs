...
stn_weeks <- lapply(1:nrow(temp),\(x) data.frame(station = temp$station[x], week = temp$minw[x]:temp$maxw[x])) %>% do.call(rbind,.)
rm(temp)
stn_sp_weeks <- expand_grid(stn_weeks, species = pull(distinct(dat, species)))
```
