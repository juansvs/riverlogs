...
There were 1096 independent events, so 1096/2374 trap days = 0.46 /day, but this varied across stations, between 0.02 and 1.0 events/day.
...
```{r}
# Find missing weeks. Large gaps are where the cameras was taken down.
dat %>% mutate(yr = year(DateTime), wk = week(DateTime)) %>% distinct(station,yr,wk) %>% arrange(station,yr,wk) %>% group_by(station) %>% mutate(wdif = lead(wk)-wk) %>% view()
```

```{r}
weekeffort <- dat %>% group_by(species) %>% filter(n()>10) %>% distinct(station,species) %>% full_join(read.csv("../Data/station_weeks.csv"))
...
freqmodels$full_RndIntSsn <- glmer(n ~ Log_lenght+Diameter+Log_to_water+River_depth+(1+season|species), data = dat_glmm_week_freq, family = poisson)
...
anova(freqmodels[[1]],freqmodels[[2]], freqmodels[[3]])
```
...
The random species intercept estimates suggest that pacas and four-eyed opossums have lower baseline activity than the rest of the species, and that tamanduas, common opossums, and coatis have higher than average. The season seems to only affect striped skunks and tamanduas. Their use of logs would increase in rainy season.
...
zifreqmodel2 <- update(zifreqmodel, ~ Log_lenght+Diameter+Log_to_water+River_depth+season+(1|species))
AIC(zifreqmodel, zifreqmodel2, freqmodels$full_RndIntSsn)
```
There isn't a major difference between including season as a random or as a fixed effect in this model. Both model have a significantly lower AIC than the Poisson model, however.

```{r}
check_zeroinflation(zifreqmodel)
check_overdispersion(zifreqmodel)
```
This model still has overdispersion and zero-inflation, the ratio of observed to predicted 0s did not change substantially (0.91 vs. 0.92). Here I include covariates in the zero-inflation model. I attempt to include the station or the species as a random effect.
...
```
This model continues to be overdispersed and zero-inflated. Next let's build the same models with a negative binomial distribution.

```{r}
freqmodel_nb <- glmer.nb(n ~ Log_lenght+Diameter+Log_to_water+River_depth+(1+season|species), data = dat_glmm_week_freq)
freqmodel_nb2 <- update(freqmodel_nb, ~Log_lenght+Diameter+Log_to_water+River_depth+season+(1|species))
...
```
All these models have lower AIC than the ones with a Poisson distribution. Out of them, the best model is the one with zero-inflation, negative binomial distribution, and season as a fixed effect. 

```{r}
check_zeroinflation(zinbfreqmod2)
check_overdispersion(zinbfreqmod2)
```
This model is not overdispersed but still seems to have excessive 0s. The ratio of zeros is not really that different from 1, even for the model with no zero-inflation. 
```{r}
summary(zinbfreqmod2)
```
