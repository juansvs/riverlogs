...
```
```{r}
dat_raw %>% filter(!species %in% c("corrupted", "nothing")) %>% 
  ggplot(aes(DateTime, station, color = camera))+stat_summary(fun.min = min, fun.max = max, geom = 'linerange', position = position_dodge(width = 0.3))
```
...
```{r}
ranef(freqmm_negbin) |> dotplot.ranef.mer()
```
...
```{r}
freqmod_nornd <- glm(n ~ Canopy_per + Log_lenght + Diameter + season + Log_to_water +      River_depth, data = dat_glmm_week_freq, family = negative.binomial(1))
AIC(freqmod_nornd,freqmm, freqmm_negbin)
```

The AIC is lower with the random intercept and the negative binomial distribution.
