...
plen_data <- ggplot(dat_freq_ogscale, aes(Log_length, n))+
  geom_point(position = position_jitter(), shape = 1)+
  labs(x = "Log length (m)", y = "Frequency (/week)")
pwid_data <- ggplot(dat_freq_ogscale, aes(Log_width, n))+
  geom_point(position = position_jitter(), shape = 1)+
  labs(x = "Log width (cm)", y = "Frequency (/week)")
pht_data <- ggplot(dat_freq_ogscale, aes(Height, n))+
  geom_point(position = position_jitter(), shape = 1)+
  labs(x = "Height above water (m)", y = "Frequency (/week)")
pdep_data <- ggplot(dat_freq_ogscale, aes(Depth, n))+
  geom_point(position = position_jitter(), shape = 1)+
  labs(x = "Stream depth (cm)", y = "Frequency (/week)")
prwid_data <- ggplot(dat_freq_ogscale, aes(River_width, n))+
  geom_point(position = position_jitter(), shape = 1)+
...
plot_grid(pfreqs2+theme(plot.margin = unit(c(0,1,0,0),"cm")),plot_grid(plen+newylims,pwid+newylims,pht+newylims,pdep+newylims,prwid+newylims, align = 'hv', labels = letters[2:6]),labels='a',rel_widths = c(0.6,1), scale = c(0.9,1))
dev.off()
```
...
pprob <- cbind(pspnewdata, mu=ypred$fit, se=ypred$se.fit) %>% 
  ...
  geom_pointrange(aes(ymin = mu-se, ymax = mu+se, color=fct_relevel(season,c("dry","rainy")),shape=fct_relevel(season,c("dry","rainy"))), position = position_dodge(0.3), show.legend = c(color=F,shape=T))+
  scale_color_manual(values = c("gold2","dodgerblue"), labels = c("Rainy","Dry"))+
  theme_classic(base_size = 14)+
  theme(axis.text.x = element_text(angle = -45, hjust=0), legend.position = 'top')+
  labs(x = "Species", y="Crossing probability", shape="Season")
