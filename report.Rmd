...
dat_glmm_week_freq <- dat %>% group_by(species) %>% filter(n()>10)%>% filter(eventcross) %>% ungroup() %>% 
  ...
  count(station,species,week) %>% mutate(month = week%/%4.3, season = if_else(week>16&week<48, "rainy","dry")) %>% left_join(rainfall) %>% left_join(log_data_scaled)
...
dat_wcovs <- dat_wcovs %>% mutate(month = month(DateTime), season = if_else(lubridate::month(DateTime) %in% c(1:4,12),"dry","rainy")) %>% left_join(rainfall)
