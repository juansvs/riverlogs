...
```{r effort table, echo=FALSE}
# Table with effort per station
dat_raw %>% filter(!species %in% c("corrupted", "nothing")) %>% group_by(station, camera) %>% summarise(mint = min(DateTime), maxt = max(DateTime),
                                            rng = difftime(max(DateTime),min(DateTime), units = "days")) %>% 
  knitr::kable(caption = "Summary of motion-activated cameras effort", digits = 0, col.names = c("Station","Camera","Start date", "End date", "Effort"))
```
...
```{r prop var, echo=FALSE, include=FALSE}
dat %>% group_by(species) %>%
  filter(n()>10) %>%
  count(station, species, eventcross) %>%
  pivot_wider(names_from = eventcross, values_from = n, values_fill = 0) %>%
  mutate(prop = `TRUE`/(`TRUE`+`FALSE`)) %>%
  ggplot(aes(fct_reorder(species, prop),prop))+geom_boxplot()+
  theme_classic(base_size = 14)+
  theme(axis.text.x = element_text(hjust = 0, angle = -45),
        plot.margin = margin(r=12, unit="pt") )+
  labs(x = "Species", y = "Proportion of log crossings")
```
...
We see that the proportion of crossings is mostly consistent across logs for the same species, as seen from the color range observed vertically. This proportion varies mainly across species. The diversity of species at each log is also different. Logs like PLOG05, PLOG06, and PLOG02 detected 15, 14, and 13 species, respectively. PLOG01 and PLOG08 in contrast each recorded only three different species.

### Frequency of crossing

```{r cross freq}
dat %>% group_by(station) %>% arrange(station,DateTime) %>% mutate(tdif = as.numeric(difftime(lead(DateTime),DateTime, units = "day"))) %>% ungroup() %>% summarise(n = n(), activity = 1/mean(tdif, na.rm = T), sd = 1/sd(tdif, na.rm=T)) 
```

We observed independent use of the logs on average once every other day. Some stations had higher activity rates, between one and 1.5 sightings per day.

```{r}
count(dat,eventcross)
```

There were 1096 independent events, so 1096/4885 trap days = 0.224 /day.

```{r}
dat %>% group_by(station) %>% mutate(stn_effort = as.numeric(max(DateTime)-min(DateTime))) %>% summarize(freq = n()/mean(stn_effort))
```

The frequency varies between 0.02 and 1.4 events/day

```{r}
dat %>% group_by(station) %>% summarise(n=n(),prop = sum(eventcross)/n()) %>% arrange(desc(prop))
```
...
pca <- log_data %>% select(-c(1,9,16)) %>% prcomp(scale. = T)
par(mfrow = c(1,2))
biplot(pca)
screeplot(pca)
```
...
It may be hard to summarize the variables in fewer than 3 components, likely due the low number of strong correlations. Because of this it makes more sense to keep the original variables, and select covariates to include based on relevance and model fit.

```{r pca weights, echo=FALSE, include=FALSE}
pca$rotation[,1:4] 
```
...
|     |     |     |     |     |     |     |     |     |     |     |     |
|-----|-----|-----|-----|-----|-----|-----|-----|-----|-----|-----|-----|
| 77  | 53  | 65  | 141 | 370 | 357 | 373 | 427 | 411 | 627 | 593 | 226 |
...
d1 <- dat %>% filter(eventcross) %>% count(station,species) %>% count(station) %>% 
  left_join(log_data)
m1 <-glm(n~Log_to_water+Diameter+Log_lenght+Canopy_per+River_depth, data=d1, family = poisson(link = 'log')) 
plot(m1)
summary(m1)
```
...
dat %>% filter(eventcross) %>% 
  mutate(week = week(DateTime)) %>% count(station,species,week) %>% # group by week of the year
  ggplot(aes(week,n))+geom_point()
```

```{r}
dat_glmm_week_freq <- dat %>% group_by(species) %>% filter(n()>10)%>% filter(eventcross) %>% ungroup() %>% 
  mutate(week = week(DateTime)) %>% count(station,species,week) %>% 
  left_join(log_data) %>% mutate(season = if_else(week>16&week<48, "rainy","dry")) %>% mutate(across(Log_lenght:Vel_conver,scale))
freqmm <- glmer(n ~ Canopy_per+Log_lenght+Diameter+season+`Stream vel`+River_depth+(1|species), data = dat_glmm_week_freq, family = poisson())
summary(freqmm)
```

```{r}
margins::margins_summary(freqmm)
```

Only three covariates seem to be influencing the frequency of crossing: canopy cover and river depth have a negative influence on crossing frequency, while log length has a positive effect.

```{r}
plot(margins::margins(freqmm))
```

```{r}
ranef(freqmm) |> dotplot.ranef.mer()
```

The random species intercept doesn't seem to have much of an influence on the frequency of crossing, so let's fit another model without the random effect.

```{r}
freqmod_nornd <- glm(n ~ Canopy_per+Log_lenght+Diameter+season+`Stream vel`+River_depth, data = dat_glmm_week_freq, family = poisson())
AIC(freqmod_nornd,freqmm)
```

The AIC is lower with the random intercept.

#### Model with monthly rainfall

```{r}
dat_glmm_week_freq_rain <- dat_glmm_week_freq %>% mutate(month = week%/%4.3) %>% left_join(rainfall) %>% mutate(across(Log_lenght:Vel_conver, scale),rain=scale(rain))
freqmm_rain <- glmer(n ~ Canopy_per+Log_lenght+Diameter+rain+`Stream vel`+River_depth+(1|species), data = dat_glmm_week_freq_rain, family = poisson())
summary(freqmm_rain)
```

```{r}
ranef(freqmm_rain) %>% dotplot.ranef.mer()
```

```{r}
plot(margins::margins(freqmm_rain))
```

This has similar results to the model with season as a categorical variable.

```{r}
performance::icc(freqmm_rain)
```

In this model the contribution of the random effect is much smaller, around 4%.

```{r}
performance::r2(freqmm_rain)
```

This model is also not explaining the variation well, the marginal R\^2, corresponding to the fixed effects, is 0.065.
...
dat_wcovs <- dat_wcovs %>% mutate(season = if_else(lubridate::month(DateTime) %in% c(1:4,12),"dry","rainy"))
# filter to keep only species with more than 10 records
dat_glmm <- dat_wcovs %>% group_by(species) %>% filter(n()>10)
# Build model with season as a covariate, a random effect of species, and multiple covariates
mm1 <- glmer(eventcross~season+Log_to_water+Diameter+Log_lenght+Canopy_per+River_depth+(1|species), data=dat_glmm, family = "binomial")
# model without season
mm2 <- glmer(eventcross~Log_to_water+Diameter+Log_lenght+Canopy_per+River_depth+(1|species), data=dat_glmm, family = "binomial")
#compare the two
AIC(mm1,mm2)
```

The second model, without season, performs better than the one with.

```{r}
performance::check_model(mm2)
```

The model performs reasonably well, although there is some underestimation around the edges and overestimation around the middle. We fit multiple models with different combinations of the covariates. The best model, according to the AIC, is one that includes the height, diameter and length of the log, as well as the river depth and canopy cover, but not the season. All the covariates, except for river depth, increased the probability of crossing the log. In other words, longer and wider logs, higher above the stream, and more covered are more likely to be crossed, while logs above deeper streams are less likely to be crossed.

```{r mm summary, echo=FALSE, include=FALSE}
summary(mm2)
```

#### Seasonality: wettest vs. driest month

An alternative approach to determining the effect of season is to create a separate model with only two months, the wettest and the driest.

This won't work, there are only a few cameras in the dry season. We could calculate instead the frequency of crosses

```{r}
plot(margins::margins(mm1))
```

#### Baseline crossing probability

```{r}
exp(fixef(mm2)[1])/(1+exp(fixef(mm2)[1]))
```

The baseline probability for all species is pretty high, we estimated it at 0.85 on average. The probability, however, will depend greatly on the species.

```{r}
ranef(mm2) |> dotplot.ranef.mer()
```

#### Effect of species

```{r ranef, echo=FALSE, fig.cap="Random effect of species on the intercept in the mixed effects model of crossing probability"}
(as.data.frame(ranef(mm2)) %>% 
  mutate(int = fixef(mm2)[1]) %>% 
  ...
  labs(x = "Species", y = "Base crossing probability")->psp)
```
...
```{r base probs, echo=FALSE, fig.cap="Baseline probability of crossing logs across rivers for different species of wild mammals and birds in the Osa Wildlife Refuge"}

data.frame(species = unique(mm2@frame$species),
           prob = predict(mm2, newdata = data.frame(species = unique(mm2@frame$species), 
           Log_to_water = median(log_data$Log_to_water), 
           Diameter = median(log_data$Diameter),
           Log_lenght = median(log_data$Log_lenght),
           Canopy_per = median(log_data$Canopy_per),
           River_depth = median(log_data$River_depth)), random.only = F,type = "response")) %>% ggplot(aes(fct_reorder(species,prob), 100*prob))+geom_point()+
  theme_classic(base_size = 16)+
  lims(y = c(0,100))+
  labs(x = "Species", 
       y = "Probability of crossing (%)")+
  theme(axis.text.x = element_text(hjust = 0, angle = -45),
                plot.margin = margin(r=16, t = 12, unit="pt") ) -> p5
p5
```

The confidence intervals for the model indicate that the inter-species variance in the intercept is significantly different from 0, and should be considered explicitly. Additionally, the 95% CIs indicate that there is a true effect of all the covariates on the probability of crossing. It is also possible that the effect of covariates varies across species. However, I tried other models with additional random effects, but none of these models performed better than the one with a random intercept.

#### Effect of covariates

```{r margins}
marfx <- margins::margins(mm2)
plot(marfx)
```
